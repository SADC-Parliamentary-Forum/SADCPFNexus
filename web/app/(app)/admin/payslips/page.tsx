...
/** Convert display period "March 2026" to input value "2026-03" for type="month". */
...
/** Convert "2026-03" to display "March 2026". */
  ...
  return (
    <div className="space-y-6 max-w-4xl">
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/50 p-4" onClick={() => setSelectedPayslip(null)}>
          <div className="card max-w-md w-full p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <dl className="space-y-2 text-sm">
              ...
              <div><dt className="text-neutral-500">Period</dt><dd className="text-neutral-700">{formatPeriod(selectedPayslip)}</dd></div>
              <div><dt className="text-neutral-500">Gross</dt><dd className="text-neutral-700">{Number(selectedPayslip.gross_amount).toLocaleString()} {selectedPayslip.currency}</dd></div>
              <div><dt className="text-neutral-500">Net</dt><dd className="text-neutral-700">{Number(selectedPayslip.net_amount).toLocaleString()} {selectedPayslip.currency}</dd></div>
            </dl>
      ...
      <div>
        ...
        <p className="page-subtitle">Bulk upload employee payslips. Files are parsed for employee number and pay period.</p>
      </div>
      ...
      <div className="card overflow-hidden">
        <div className="card-header">
          ...
          <button type="button" onClick={() => fetchList()} className="text-xs font-medium text-primary hover:underline">
            Refresh
          </button>
        </div>
        {/* Filters */}
        <div className="px-4 py-3 border-b border-neutral-200 flex flex-wrap gap-3 items-end">
          ...
          <button type="button" onClick={() => fetchList()} className="btn-primary px-4 py-2 text-sm">
            Apply filters
          </button>
        </div>
        {loadingList ? (
          ...
          <div className="p-8 text-center text-sm text-neutral-500">No payslips match the filters. Upload files below or adjust filters.</div>
        ) : (
            ...
            <table className="data-table">
              ...
              <tbody>
                {uploadedList.map((p) => (
                  ...
                  >
                    <td className="font-medium text-neutral-900">{p.user?.name ?? p.user?.email ?? "—"}</td>
                    <td className="font-mono text-neutral-600 text-sm">{p.user?.employee_number ?? "—"}</td>
                    <td className="text-neutral-700 whitespace-nowrap">{formatPeriod(p)}</td>
                    <td className="text-right text-neutral-700">{Number(p.gross_amount).toLocaleString()}</td>
                    <td className="text-right text-neutral-700">{Number(p.net_amount).toLocaleString()}</td>
                    <td className="text-neutral-600">{p.currency}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.id}>
                    <td>
                      <div className="flex items-center gap-2">
                        <span className="material-symbols-outlined text-neutral-400 text-[16px]">
                          {row.filename.endsWith(".pdf") ? "picture_as_pdf" : "table_chart"}
                        </span>
                    ...
                    <td>
                      <input
                        className="w-full rounded-md border border-neutral-200 bg-neutral-50 px-2 py-1.5 text-xs font-mono focus:border-primary focus:ring-1 focus:ring-primary outline-none"
                        value={row.employeeNum}
                        onChange={(e) => updateRow(row.id, "employeeNum", e.target.value)}
                      />
                    </td>
                    <td>
                      <input
                        type="month"
                        className="w-full rounded-md border border-neutral-200 bg-neutral-50 px-2 py-1.5 text-xs focus:border-primary focus:ring-1 focus:ring-primary outline-none min-w-[140px]"
                        value={periodToMonthValue(row.period)}
                        onChange={(e) => updateRow(row.id, "period", monthValueToPeriod(e.target.value))}
                        title="Select pay period (month and year)"
                      />
                    </td>
                    <td>
      ...
      {isAdmin && rows.length === 0 && (
        <div className="card p-6">
          <h3 className="text-sm font-semibold text-neutral-700 mb-2">Filename Parsing Rules</h3>
          <ul className="space-y-1.5 text-xs text-neutral-500">
            <li className="flex items-start gap-2"><span className="material-symbols-outlined text-[14px] text-primary mt-0.5">check_circle</span>Employee number: extracted from <code className="bg-neutral-100 px-1 rounded">EMP\d+</code> pattern, or first segment before <code className="bg-neutral-100 px-1 rounded">_</code> or <code className="bg-neutral-100 px-1 rounded">-</code></li>
            <li className="flex items-start gap-2"><span className="material-symbols-outlined text-[14px] text-primary mt-0.5">check_circle</span>Period: detected from month names (e.g. March) + year (e.g. 2026) in filename</li>
            <li className="flex items-start gap-2"><span className="material-symbols-outlined text-[14px] text-primary mt-0.5">check_circle</span>All fields are editable before uploading</li>
          </ul>
        </div>
      )}
    </div>
