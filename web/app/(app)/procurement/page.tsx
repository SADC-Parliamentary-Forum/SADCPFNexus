...
const categoryColors: Record<string, string> = {
  ...
  services: "text-purple-700 bg-purple-50 border-purple-200",
  works:    "text-orange-700 bg-orange-50 border-orange-200",
};

const methodColors: Record<string, string> = {
  quotation: "text-teal-700 bg-teal-50",
  tender:    "text-indigo-700 bg-indigo-50",
  direct:    "text-neutral-700 bg-neutral-100",
};
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          ...
          { label: "Pending Approval",   value: requests.filter((r) => r.status === "submitted").length.toString(), icon: "pending_actions",color: "text-amber-600", bg: "bg-amber-50"  },
          { label: "Approved",           value: requests.filter((r) => r.status === "approved").length.toString(),  icon: "check_circle",   color: "text-green-600", bg: "bg-green-50"  },
          { label: "Total Value",        value: totalValue > 0 ? `${currency} ${(totalValue / 1000).toFixed(0)}k` : "—", icon: "payments", color: "text-purple-600",bg: "bg-purple-50" },
        ].map((s) => (
      ...
      {loading ? (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-4 flex-1 min-w-0">
                    <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-purple-50">
                      <span className="material-symbols-outlined text-purple-600 text-[20px]">shopping_cart</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm font-semibold text-neutral-900 truncate">{req.title}</p>
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className="text-[10px] text-neutral-400 uppercase tracking-wide">Est. Value</p>
        ...
        <div className="card p-16 text-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-neutral-100 mx-auto">
            <span className="material-symbols-outlined text-4xl text-neutral-300">shopping_cart</span>
          </div>
          <p className="mt-4 text-sm font-semibold text-neutral-600">No procurement requests found</p>
