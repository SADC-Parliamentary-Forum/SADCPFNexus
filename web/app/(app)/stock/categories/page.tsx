...
export default function StockCategoriesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editId) {
        await stockCategoriesApi.update(editId, { name, code, sort_order: form.sort_order });
        toast("success", "Category updated");
      } else {
        await stockCategoriesApi.create({ name, code, sort_order: form.sort_order });
        toast("success", "Category created");
      }
      const res = await stockCategoriesApi.list();
      setCategories(res.data.data ?? []);
      setEditId(null);
      setForm({ name: "", code: "", sort_order: 0 });
      setShowForm(false);
    } catch (err: unknown) {
      const msg = (err as ApiError)?.response?.data?.message || "Failed to save category.";
      setError(msg);
      toast("error", msg);
    } finally {
      setSaving(false);
  ...
  const startEdit = (cat: StockCategory) => {
    setEditId(cat.id);
    setForm({ name: cat.name, code: cat.code, sort_order: cat.sort_order ?? 0 });
    setShowForm(true);
  ...
  const handleDelete = async (id: number, name: string) => {
    if (!(await confirm({
      ...
      message: `Delete "${name}"? Only allowed if no stock items use this category.`,
      variant: "danger",
    }))) return;
    try {
      await stockCategoriesApi.delete(id);
      setCategories((prev) => prev.filter((c) => c.id !== id));
      toast("success", "Category deleted");
    } catch (err: unknown) {
      const msg = (err as ApiError)?.response?.data?.message || "Cannot delete category (may be in use).";
      toast("error", msg);
    }
  };
        ...
        <div className="card border-primary/20 p-5 space-y-4">
          <div className="flex items-center gap-3">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary/10">
              <span className="material-symbols-outlined text-primary text-[18px]">{editId ? "edit" : "category"}</span>
            </div>
            <h3 className="text-sm font-semibold text-neutral-900">{editId ? "Edit Category" : "New Category"}</h3>
          </div>
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <button onClick={handleSave} disabled={!form.name.trim() || !form.code.trim() || saving} className="btn-primary">
              {saving
      ...
      <div className="card overflow-hidden">
        ...
        {loading ? (
          ...
          <ul className="divide-y divide-neutral-100">
            {categories.map((cat) => (
              <li key={cat.id} className="flex items-center justify-between gap-4 px-5 py-4 hover:bg-neutral-50/80 transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-mono text-neutral-500">{cat.code}{typeof cat.items_count === "number" ? ` · ${cat.items_count} item(s)` : ""}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button type="button" onClick={() => startEdit(cat)} className="p-2 rounded-lg text-neutral-500 hover:bg-neutral-100 hover:text-neutral-700" aria-label="Edit">
                    <span className="material-symbols-outlined text-[18px]">edit</span>
                  </button>
                  <button type="button" onClick={() => handleDelete(cat.id, cat.name)} className="p-2 rounded-lg text-neutral-500 hover:bg-red-50 hover:text-red-600" aria-label="Delete">
