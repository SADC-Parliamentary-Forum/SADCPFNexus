...
export default function PeopleSkillsPage() {
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => (await peopleAuthorityApi.listSkills()).data,
  });
    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-semibold text-neutral-900">Skills and competence directory</h1>
        </div>
        <Link href="/people" className="text-sm underline">Hub</Link>
      </div>
      {isLoading && <p className="text-sm text-neutral-500">Loading…</p>}
      {isError && <p className="text-sm text-red-600">Unable to load skills.</p>}
      {data && (
