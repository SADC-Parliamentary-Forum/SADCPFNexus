    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-1">
          ...
          <span>/</span>
          <span className="text-gray-700 font-medium">Liquidate</span>
        </div>
        <h2 className="text-xl font-bold text-gray-900">Submit Liquidation</h2>
        <p className="text-sm text-gray-500 mt-0.5">Record all expenditures and attach supporting receipts.</p>
      </div>
      ...
      <div className="rounded-xl bg-white border border-gray-100 shadow-card p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-gray-900">Expenditure Lines</h3>
        ...
        <div className="space-y-3">
          {lines.map((line, i) => (
            <div key={line.id} className="rounded-lg border border-gray-100 bg-gray-50 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Line {i + 1}</span>
                {lines.length > 1 && (
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2 space-y-1">
                  ...
                  <input
                    className="w-full rounded-md border border-gray-200 bg-white px-2.5 py-2 text-sm text-gray-900 placeholder-gray-400 focus:border-primary focus:ring-1 focus:ring-primary outline-none"
                ...
                <div className="space-y-1">
                  <label className="block text-[11px] font-medium text-gray-600">Vendor / Supplier</label>
                  <input
                    className="w-full rounded-md border border-gray-200 bg-white px-2.5 py-2 text-sm text-gray-900 placeholder-gray-400 focus:border-primary focus:ring-1 focus:ring-primary outline-none"
                ...
                <div className="space-y-1">
                  <label className="block text-[11px] font-medium text-gray-600">Receipt Date</label>
                  <input
                    type="date"
                    className="w-full rounded-md border border-gray-200 bg-white px-2.5 py-2 text-sm text-gray-900 focus:border-primary focus:ring-1 focus:ring-primary outline-none"
                    value={line.receipt_date}
                    onChange={(e) => updateLine(line.id, "receipt_date", e.target.value)}
                  />
                </div>
                <div className="space-y-1">
                  <label className="block text-[11px] font-medium text-gray-600">Amount ({approved.currency}) <span className="text-red-500">*</span></label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      min="0"
                      step="0.01"
                      className="w-full rounded-md border border-gray-200 bg-white pl-6 pr-2.5 py-2 text-sm text-gray-900 placeholder-gray-400 focus:border-primary focus:ring-1 focus:ring-primary outline-none"
                      placeholder="0.00"
                      value={line.amount}
        ...
        <div className="rounded-lg bg-gray-50 border border-gray-100 p-4 space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-bold text-gray-900">{approved.currency} {totalSpent.toFixed(2)}</span>
          </div>
          <div className="flex justify-between text-sm border-t border-gray-200 pt-2 mt-1">
            <span className="text-gray-500">Balance {balance >= 0 ? "to Return" : "Overspent"}</span>
      ...
      {/* Actions */}
      <div className="flex items-center justify-between">
        <Link href={`/imprest/${params.id}`} className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-primary transition-colors">
