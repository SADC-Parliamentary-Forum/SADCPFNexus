    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 text-sm text-neutral-500 mb-1">
          ...
          <span>/</span>
          <span className="text-neutral-700 font-medium">Salary Advance Request</span>
        </div>
        <h2 className="text-xl font-bold text-neutral-900">Request Salary Advance</h2>
        <p className="text-sm text-neutral-500 mt-0.5">Subject to policy limits and approval. Maximum advance is 3× your monthly gross.</p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 space-y-5">
          ...
          <div className="rounded-xl bg-white border border-neutral-100 shadow-card p-5 space-y-3">
            <h3 className="text-sm font-semibold text-neutral-900">Advance Type <span className="text-red-500">*</span></h3>
            <div className="space-y-2">
              {advanceTypes.map((t) => (
                <button key={t.value} type="button" onClick={() => set("advance_type", t.value)}
                  className={`w-full flex items-center gap-3 rounded-xl border p-3 text-left transition-all ${form.advance_type === t.value ? "border-primary bg-primary/5 ring-1 ring-primary" : "border-neutral-200 hover:border-neutral-300"}`}>
                  <div className="h-8 w-8 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0">
                    <span className={`material-symbols-outlined text-[18px] ${form.advance_type === t.value ? "text-primary" : "text-neutral-400"}`}>{t.icon ?? "payments"}</span>
                  </div>
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-neutral-900">{t.label}</p>
                    {t.desc && <p className="text-xs text-neutral-500">{t.desc}</p>}
                  </div>
          ...
          <div className="rounded-xl bg-white border border-neutral-100 shadow-card p-5 space-y-4">
            <h3 className="text-sm font-semibold text-neutral-900">Amount & Repayment</h3>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="block text-xs font-medium text-neutral-700">Amount Requested ({defaultCurrency}) <span className="text-red-500">*</span></label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400 text-sm">{defaultCurrency === "NAD" ? "N$" : defaultCurrency}</span>
                  <input type="number" min="100" max={MAX_ADVANCE} step="100"
                    className="w-full rounded-lg border border-neutral-200 bg-neutral-50 pl-8 pr-3 py-2.5 text-sm focus:border-primary focus:ring-1 focus:ring-primary outline-none"
                    placeholder="0.00"
                    value={form.amount}
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-medium text-neutral-700">Repayment Period</label>
                <select className="w-full rounded-lg border border-neutral-200 bg-neutral-50 px-3 py-2.5 text-sm focus:border-primary focus:ring-1 focus:ring-primary outline-none"
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-medium text-neutral-700">Purpose <span className="text-red-500">*</span></label>
              <input className="w-full rounded-lg border border-neutral-200 bg-neutral-50 px-3 py-2.5 text-sm focus:border-primary focus:ring-1 focus:ring-primary outline-none"
                placeholder="Brief description of the purpose"
                value={form.purpose} onChange={(e) => set("purpose", e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <label className="block text-xs font-medium text-neutral-700">Detailed Justification <span className="text-red-500">*</span></label>
              <textarea rows={3}
        ...
        <div className="lg:w-64 space-y-4">
          <div className="rounded-xl bg-white border border-neutral-100 shadow-card p-4 space-y-3">
            <h4 className="text-xs font-semibold text-neutral-500 uppercase tracking-wider">Repayment Summary</h4>
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-neutral-900">{defaultCurrency} {amount.toLocaleString()}</span>
              </div>
              <div className="flex justify-between border-t border-neutral-50 pt-2 mt-1">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <button onClick={() => handleSubmit(true)} disabled={submitting} className="rounded-lg border border-neutral-200 bg-white px-4 py-2 text-sm font-semibold text-neutral-700 hover:bg-neutral-50 transition-colors disabled:opacity-50">
            Save Draft
          </button>
