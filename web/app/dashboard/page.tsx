    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      {showCustomize && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="rounded-2xl bg-white dark:bg-neutral-800 p-6 max-w-sm w-full shadow-2xl border border-neutral-100 dark:border-neutral-700">
          ...
          {activityLoading ? (
            <div className="divide-y divide-neutral-50 dark:divide-neutral-800">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="px-5 py-3.5 flex gap-3 items-center">
                  <div className="h-8 w-8 rounded-full bg-neutral-100 dark:bg-neutral-700/40 animate-pulse" />
                  <div className="flex-1 space-y-1.5">
                    <div className="h-3 w-40 bg-neutral-100 dark:bg-neutral-700/40 animate-pulse rounded" />
                    <div className="h-2.5 w-24 bg-neutral-100 dark:bg-neutral-700/40 animate-pulse rounded" />
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="divide-y divide-neutral-50 dark:divide-neutral-800">
              {activity.map((a) => {
                ...
                return (
                  <Link key={`${a.module}-${a.id}`} href={a.href} className="flex items-center gap-3 px-5 py-3 hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition-colors group">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-neutral-600 uppercase font-medium">{a.module}</span>
                      </div>
                      <p className="text-[11px] text-neutral-600">{formatDateShort(a.date)}</p>
                    </div>
          ...
          {upcomingEvents.length === 0 ? (
            ...
            <div className="divide-y divide-neutral-50 dark:divide-neutral-800">
              {upcomingEvents.map((e) => {
                ...
                const content = (
                  <>
                    <div className={`flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg text-[12px] font-bold ${EVENT_COLOR[e.type] ?? "bg-neutral-100 dark:bg-neutral-700/40 text-neutral-600 dark:text-neutral-300"}`}>
                      {e.type === "birthday"
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-neutral-600">{formatDateShort(e.date)}</p>
                    </div>
                  </>
                );
                return isSocial ? (
                  ...
                  <Link key={e.id} href={`/workplan/${e.id}`} className="flex items-center gap-3 px-4 py-3 hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition-colors">{content}</Link>
                );
              })}
            </div>
          )}
        ...
        {urgentAssignments.length > 0 ? (
          <div className="divide-y divide-neutral-50 dark:divide-neutral-800">
            {urgentAssignments.map((a) => {
              ...
              return (
                <Link key={a.id} href={`/assignments/${a.id}`} className="flex items-start gap-3 px-5 py-3 hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition-colors group">
                  <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-red-50 dark:bg-red-900/20 mt-0.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                      <span className="text-[10px] font-mono text-neutral-600">{a.reference_number}</span>
                    ...
                    {showFullAssignments && a.assignee && (
                      <p className="text-[10px] text-neutral-600">{a.assignee.name}</p>
                    )}
                  </div>
                  <div className="flex-shrink-0 text-right">
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-[10px] text-neutral-600">{a.progress_percent}%</span>
                    </div>
                  </div>
                </Link>
              );
          ...
          <div className="px-5 py-7 text-center">
            ...
            <p className="text-xs text-neutral-600 mt-1.5">No urgent assignments</p>
          </div>
        )}
      </div>
      )}
      ...
      {modulesToShow.length > 0 && <div>
        <h3 className="text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-3">All Modules</h3>
