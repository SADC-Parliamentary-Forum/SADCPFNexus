...
class MyAssignedAssetsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<MyAssignedAssetsScreen> createState() =>
      _MyAssignedAssetsScreenState();
}

class _MyAssignedAssetsScreenState
    extends ConsumerState<MyAssignedAssetsScreen> {
  String _filter = 'All';
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final res = await dio.get<Map<String, dynamic>>('/assets',
          queryParameters: {'assigned_to': 'me', 'per_page': 100});
      if (!mounted) return;
  ...
  List<Map<String, dynamic>> get _filtered {
    ...
    return _assets
        .where((a) => (a['category'] as String? ?? '').toLowerCase() == cat)
        .toList();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return StitchScreen(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(children: [
              Expanded(
                  child: _summaryItem(
                      'Total Assets', '${_assets.length}', AppColors.primary)),
              _divider(),
              Expanded(
                  child: _summaryItem(
                      'Active',
                      '${_assets.where((a) => a['status'] == 'Active').length}',
                      AppColors.success)),
              _divider(),
              Expanded(
                  child: _summaryItem(
          ...
          Expanded(
            child: _loading
                ? const StitchLoadingState(label: 'Loading assigned assets')
                : _error != null
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => context.push('/assets/condition-report'),
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: Icon(_categoryIcon(asset['category'] as String?),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 2),
                    Text(
                        'ID: ${asset['asset_code'] ?? asset['id']}  ·  ${_categoryLabel(asset['category'] as String?)}',
                        style: const TextStyle(
          ...
          Row(
            children: [
              Expanded(
                  child: _infoChip(
                      Icons.calendar_today_outlined, 'Issued', issuedAt)),
