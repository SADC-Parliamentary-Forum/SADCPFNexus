...
class ProgrammeAttachmentController extends Controller
{
    ...
    public function store(Request $request, Programme $programme): JsonResponse
    {
        ...
        $attachment = $this->bridge->storeAttachment($request->user(), $programme, $file, [
    ...
    public function destroy(Request $request, Programme $programme, Attachment $attachment): JsonResponse
    {
        $this->ensureProgrammeTenant($programme);
        if ($attachment->attachable_type !== Programme::class || (int) $attachment->attachable_id !== (int) $programme->id) {
            abort(404);
        }
        // PRD §126: unlink ≠ delete — bridge unlinks without destroying ManagedDocument evidence.
    ...
    public function download(Programme $programme, Attachment $attachment): StreamedResponse|JsonResponse
    {
        ...
        if (! $attachment->storage_path || ! Storage::disk($disk)->exists($attachment->storage_path)) {
            return response()->json(['message' => 'File not found.'], 404);
        }

        return response()->streamDownload(
            function () use ($attachment, $disk) {
                $stream = Storage::disk($disk)->readStream($attachment->storage_path);
