...
class SupplierRegistrationController extends Controller
{
    ...
    public function register(Request $request): JsonResponse
    {
        ...
        $data = $request->validate([
            ...
            'document_expiry_dates' => ['nullable', 'array', 'max:15'],
        ...
        foreach ($request->file('documents', []) as $index => $file) {
            ...
            $expiryDate = $request->input("document_expiry_dates.{$index}");
            try {
                $this->documents->storeForVendor($vendor, $file, $documentType, $user, [
                    'expiry_date' => filled($expiryDate) ? $expiryDate : null,
                ]);
            } catch (\Illuminate\Validation\ValidationException $e) {
                $messages = collect($e->errors())->flatten()->filter()->values()->all();
                throw \Illuminate\Validation\ValidationException::withMessages([
                    "documents.{$index}" => $messages !== [] ? $messages : ['The uploaded file type is not allowed.'],
                ]);
            }
        }
